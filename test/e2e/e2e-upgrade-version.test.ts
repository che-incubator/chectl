...
describe('Test Che upgrade', () => {
  describe('Prepare latest stable Che', () => {
    it(`Deploy Che using operator installer and self signed certificates`, async () => {
      ...
      // see github workflow
      let deployCommand = `chectl server:deploy --batch --platform=${PLATFORM} --chenamespace=${NAMESPACE} --k8spodwaittimeout=600000 --k8spodreadytimeout=600000 --telemetry=off`
  ...
  describe('Test Che update', () => {
    it('Update Eclipse Che Version', async () => {
      ...
      await helper.runCliCommand(binChectl, ['server:update', '-y', `-n ${NAMESPACE}`])
      await helper.runCliCommand(binChectl, ['server:start', `-n ${NAMESPACE}`, '--k8spodwaittimeout=600000', '--k8spodreadytimeout=600000'])
