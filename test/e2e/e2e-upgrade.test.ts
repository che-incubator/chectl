...
describe('Test Che upgrade', () => {
  ...
  describe('Prepare latest stable Che', () => {
    it(`Deploy Che using ${INSTALLER} installer and self signed certificates`, async () => {
      ...
      const latestStableCheTag = await helper.getLatestChectlProdRelease()
  ...
  describe('Test Che update', () => {
    it('Update Eclipse Che Version', async () => {
      ...
      await helper.waitForCheServerImageTag(helper.getLatestRelease(), UPDATE_CHE_TIMEOUT_MS)
    })

    it('Check updated Che version', async () => {
      await helper.waitForVersionInCheCR(helper.getLatestRelease(), CHE_VERSION_TIMEOUT_MS)
    })
  })
